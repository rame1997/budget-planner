...
class _loginScreenState extends State<LoginScreen> with Helpers{
  ...
  bool checkData() {
    if (_emailTextEditingController.text.isNotEmpty && _pinTextEditingController.text.isNotEmpty) {
      return true;
    }
  ...
  Future login() async {
    User? user = await UserDbController().login(email: _emailTextEditingController.text, pin: _pinTextEditingController.text);
    print('rami');

    if (user != null) {
      showSnackBar(context, message: AppLocalizations.of(context)!.login_success);
      SharedPrefController().save(user);
      Navigator.pushNamedAndRemoveUntil(context, '/bottom_bavigation_bar_screen', (route) => false);
    } else{
  ...
  User get user {
    User user = User();
    user.name = _pinTextEditingController.text;
